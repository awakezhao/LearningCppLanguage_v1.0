...
int CacheConn::Init()
{
    ...
    if (cur_time < m_last_connect_time + 1)
    {
        printf("cur_time:%lu, m_last_connect_time:%lu", cur_time, m_last_connect_time);
        return 1;
    }
    ...
    m_pContext = redisConectWithTimeout(m_server_ip.c_str(), m_server_port, timeout);

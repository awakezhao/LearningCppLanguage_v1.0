...
#define EXCEPTI_MESSAGE_LENGTH 512
...
ntyException TimeoutExceptionn = {"TimeoutException"};
...
typedef struct _ntyExceptionFrame {
    ...
    char message[EXCEPTI_MESSAGE_LENGTH + 1];
...
void ntyExceptionThrow(ntyException *excep, const char *func, const char *file, int line, const char *cause, ...) {
    ...
    if (frame) {
        ...
        if (cause) {
            va_start(ap, cause);
            vsnprintf(frame->message, EXCEPTI_MESSAGE_LENGTH, cause, ap);
            va_end(ap);
        }
        ...
        va_start(ap, cause);
        vsnprintf(message, EXCEPTI_MESSAGE_LENGTH, cause, ap);
        va_end(ap);
